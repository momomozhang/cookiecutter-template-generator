...
function generate-project {
    ...
    echo "=== Running cookiecutter command ==="
    cookiecutter ./ \
    ...
    if [ $cookiecutter_exit_code -ne 0 ]; then
        echo "ERROR: cookiecutter command failed with exit code $cookiecutter_exit_code"
        exit $cookiecutter_exit_code
    fi
    echo "Cookiecutter command completed successfully"
    ...
    echo "=== Navigating to generated project ==="
    cd $(ls)
...
function clean {
    rm -rf dist build coverage.xml test-reports sample/ cookiecutter-test-config.json tests/cookiecutter-*.json
    find . \
      -type d \
      \( \
        -name "*cache*" \
...
EOF
    ...
    gh pr create \
        --title "feat: populated from cookiecutter template generator" \
        --body "This PR was generated by \`cookiecutter template generator\`" \
        --base main \
